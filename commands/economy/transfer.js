...
module.exports = class TransferCommand extends Command {
	...
	async run(msg, { user, amount }) {
		...
		if (amount < rules.minWager) {
			...
			return msg.reply(
				`❌ You need to have at least \`${amount}\` ${
					rules.currencyPlural
				}. Your balance is \`${await diceAPI.getBalance(msg.author.id)}\`.`
			);
		...
		// Tell the sender
		return msg.reply(
			`📤 Transferred \`${amount}\` ${rules.currencyPlural} to <@${
				user.id
			}>. You now have a balance of \`${await diceAPI.getBalance(msg.author.id)}\` ${
				rules.currencyPlural
			}.`
		);
	}
