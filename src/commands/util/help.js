      ...
      if (commands.length === 1) {
        ...
        const embed = new MessageEmbed({
          ...
          fields: [
            ...
            {
              name: "Group",
              value: `${commands[0].group.name} (\`${commands[0].groupID}:${
                commands[0].memberName
              }\`)`
            }
          ]
        });
        ...
        if (command.examples) {
          const prettyExamples = command.examples.map(
            example => `\`${example}\``
          );
        ...
        try {
          ...
          messages.push(
            await msg.reply(
              "An unknown error occured when sending the help message."
            )
          );
        }
        return messages;
