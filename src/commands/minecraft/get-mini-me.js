...
module.exports = class GetMinecraftMiniMeCommand extends Command {
  ...
  run(msg, { username, transparency }) {
    ...
    const embed = new MessageEmbed({
      author: {
        name: username,
        url: `http://minecraftskinavatar.com/customize?id=${encodeURIComponent(
          username
        )}&source=minecraft`,
    ...
    if (transparency) {
      // eslint-disable-next-line max-len
      embed.setImage(
        `http://avatar.yourminecraftservers.com/avatar/trnsp/steve/tall/128/${encodeURIComponent(
          username
        )}.png`
      );
    } else {
      // eslint-disable-next-line max-len
      embed.setImage(
        `http://avatar.yourminecraftservers.com/avatar/rad/steve/tall/128/${encodeURIComponent(
          username
        )}.png`
      );
    }
